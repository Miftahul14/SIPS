...
@login_required(login_url='HalamanLogin')
@ijinkan_pengguna(yang_diizinkan=['administrator']) 
def sanksi_pelanggaran(request): 
    sanksi_list = Daftar_Sanksi.objects.all().order_by('-id')
    context = {
        'menu': 'pelanggaransiswa',
        'submenu': 'sanksi_pelanggaran',
        'judul' : 'Sanksi Pelanggaran',
        'sanksi_list' : sanksi_list,
        }
    return render(request, 'sanksi_pelanggaran.html', context)

def tambah_sanksi(request):
    if request.method == 'POST':
        poin = request.POST['poin']
        sanksi = request.POST['sanksi']
        sanksi_list = Daftar_Sanksi(poin = poin, sanksi = sanksi)
        sanksi_list.save()
        messages.success(request, f'{sanksi} Berhasil Ditambahkan')
        return redirect("sanksi_pelanggaran")
    return render(request, "sanksi_pelanggaran.html")

def edit_sanksi(request, id):
    sanksi_list = Daftar_Sanksi.objects.get(id=id)
    if request.method == 'POST':
        sanksi_list.poin = request.POST['poin']
        sanksi_list.sanksi = request.POST['sanksi']
        sanksi_list.save()
        messages.success(request, 'berhasil Diperbarui ')
        return redirect('sanksi_pelanggaran')
...
def hapus_sanksi(request,id):
    sanksi_list = get_object_or_404(Daftar_Sanksi, id=id)
    sanksi_list.delete()
    messages.success(request, 'Berhasil Dihapus')
    return redirect("sanksi_pelanggaran")
